...
public class TechnicalDetail extends AppCompatActivity {
    RecyclerView rvEvent;
    private List<Model> technicalEvents = new ArrayList<>();
    ...
    private void LoadJson() {
        ...
        Call<EventData> call = api.getTechnicalEvents();

        call.enqueue(new Callback<EventData>() {
            @Override
            public void onResponse(Call<EventData> call, Response<EventData> response) {
                technicalEvents = response.body().getEventList();
                eventAdapter = new event_adapter(technicalEvents, TechnicalDetail.this);
